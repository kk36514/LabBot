# -*- coding: utf-8 -*-
"""LabBot main.py — Discord intel bot (Render / Colab compatible)."""
...
# Migrations — add missing columns one by one
...
async def report(
    ...
    try:
        cursor = conn.execute(
            "INSERT INTO reports (org, action, location, notes, reported_by, anonymous, tags, lat, lon, timestamp) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (org, action, location, notes, reported_by, int(anonymous), tags or "", lat, lon, ts)
        )
        conn.commit()
...
async def track(interaction: discord.Interaction, org: str = DEFAULT_ORG, limit: int = 10):
    ...
    if not await check_access(interaction):
        return
    rows = conn.execute(
        "SELECT id, action, location, timestamp FROM reports WHERE lower(org) = lower(?) ORDER BY id DESC LIMIT ?",
        (org, limit)
    ).fetchall()
    if not rows:
...
async def summary(interaction: discord.Interaction, org: str = DEFAULT_ORG):
    ...
    if not await check_access(interaction):
        return
    rows = conn.execute(
        "SELECT action, COUNT(*) FROM reports WHERE lower(org) = lower(?) GROUP BY action", (org,)
    ).fetchall()
    if not rows:
...
async def location(interaction: discord.Interaction, loc: str):
    ...
    if not await check_access(interaction):
        return
    rows = conn.execute(
        "SELECT id, org, action, timestamp FROM reports WHERE location LIKE ? ORDER BY id DESC",
        (f"%{loc}%",)
    ).fetchall()
    if not rows:
...
async def timeline(interaction: discord.Interaction, org: str = DEFAULT_ORG):
    await interaction.response.defer(ephemeral=True)
    rows = conn.execute(
        "SELECT timestamp, action, notes FROM reports WHERE lower(org) = lower(?) ORDER BY timestamp DESC LIMIT 20",
        (org,)
    ).fetchall()
    if not rows:
...
async def stats(interaction: discord.Interaction):
    ...
    top_reporter = conn.execute(
        "SELECT reported_by, COUNT(*) as c FROM reports GROUP BY reported_by ORDER BY c DESC LIMIT 1"
    ).fetchone()

...
async def main():
    token = os.environ.get("BOT_API") # Get token from environment variables
    if not token: # If not set as env variable, try Colab secrets
        try:
            from google.colab import userdata
            token = userdata.get("DISCORD_BOT_TOKEN")
        except Exception:
            print("DISCORD_BOT_TOKEN or BOT_API not found in environment or Colab secrets.")
            return
...
if __name__ == "__main__":
    # Run asyncio loop safely without throwing top-level awaits errors across Colab & Render
    try:
        try:
